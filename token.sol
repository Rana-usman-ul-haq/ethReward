...
contract CoinDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("Coin_Dividend_Tracker", "Coin_Dividend_Tracker") {
        claimWait = 100;
    ...
    function withdrawDividend() public pure override {
        require(false, "Coin_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Coin_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
...
contract TestToken is ERC20, Ownable, SafeToken {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Test Coin", "Test") {
    	...
    	dividendTracker = new CoinDividendTracker();
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ERC20: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ERC20: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ERC20: Cannot update gasForProcessing to same value");
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 tokensToAddLiquidityWith = contractTokenBalance.div(totalSellFees.mul(2)).mul(sellLiquidityFee);
        ...
        uint256 bnbToAddLiquidityWith = deltaBalance.mul(sellLiquidityFee).div(totalSellFees.mul(2).sub(sellLiquidityFee));
        
        // add liquidity to uniswap
        addLiquidity(tokensToAddLiquidityWith, bnbToAddLiquidityWith);
        // worthy marketing fee
        uint256 marketingAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalSellFees.sub(sellLiquidityFee)).mul(sellMarketingFee);
    ...
    function changeBuyFees(uint256 _rewardFee, uint256 _liquidityFee, uint256 _marketingFee) public onlyOwner {
        require(_rewardFee.add(_liquidityFee).add(_marketingFee) <= 19);
        buyRewardFee = _rewardFee;
        buyLiquidityFee = _liquidityFee;
        buyMarketingFee = _marketingFee;
        totalBuyFees = _rewardFee.add(_liquidityFee).add(_marketingFee);
    }


     function changeSellFees(uint256 _rewardFee, uint256 _liquidityFee, uint256 _marketingFee) public onlyOwner {
        require(_rewardFee.add(_liquidityFee).add(_marketingFee) <= 19);
        sellRewardFee = _rewardFee;
        sellLiquidityFee = _liquidityFee;
        sellMarketingFee = _marketingFee;
        totalSellFees = _rewardFee.add(_liquidityFee).add(_marketingFee);
    }
